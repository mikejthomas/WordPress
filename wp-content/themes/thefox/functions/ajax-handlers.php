...
if (!function_exists('get_portfolio_works')) {
  ...
  WPBMap::addAllMappedShortcodes(); 
        ...
        $target = $_POST['target'];	
        $port_order = $_POST['order'];	        
		$port_orderby = $_POST['orderby'];	
...
if ( class_exists("Vc_base") ) {
  WPBMap::addAllMappedShortcodes(); 
        ...
        $blog_order = $_POST['order'];
        $blog_orderby = $_POST['orderby'];
...
$n_category = explode(',', $category);
	...
	query_posts('category_name='.$category.'&order='.$blog_order.'&orderby='.$blog_orderby.'&posts_per_page=1');
		}else{
	query_posts('order='.$blog_order.'&orderby='.$blog_orderby.'&posts_per_page=1');
...
$n_category = explode(',', $category);
	query_posts('offset='.$last_post.'&category_name='.$category.'&order='.$blog_order.'&orderby='.$blog_orderby.'&posts_per_page=1');
		}else{
	query_posts('offset='.$last_post.'&order='.$blog_order.'&orderby='.$blog_orderby.'&posts_per_page=1');
...
if (!function_exists('get_staff_posts')) {
    function get_staff_posts()
    {
        ...
        $now_open_works = $_POST['now_open_works'];
        $works_per_load = $_POST['works_per_load'];
        $staff_order = $_POST['order'];
        $staff_orderby = $_POST['orderby'];
...
<div data-category="<?php echo esc_attr($echoallterm); ?>" class="staff_post rd_staff_p05 ajax_post <?php echo esc_attr($echoallterm); ?>">
  ...
  <div class="member-info">
    ...
    <span class="position" ><?php echo !empty( $position ) ? $position : ''; ?></span>
    <div class="member_desc"><?php $small_desc = substr($desc,0,120); $small_desc .= '...';  echo !empty( $small_desc ) ? $small_desc : ''; ?><a href="<?php echo $staff_link ?>" target="_blank" class="staff_button"><?php echo __('Read More', 'thefoxwp'); ?></a></div>
  </div>
...
<?php }
   ...
   <div data-category="<?php echo esc_attr($echoallterm); ?>" class="carousel_recent_post team-member staff_post ajax_post <?php echo esc_attr($echoallterm); ?>">
    ...
    <div class="bw-wrapper">       <a href="<?php echo $staff_link ?>" target="<?php echo esc_attr($l_target); ?>"><?php if($html_template == 'rstaff_02'){  echo the_post_thumbnail( array(586, 440), array('title' => "")); }else{ echo the_post_thumbnail( 'staff_tn', array('title' => "")); } ?> 
