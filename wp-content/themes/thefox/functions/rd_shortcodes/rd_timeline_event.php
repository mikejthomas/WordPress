...
if (!function_exists('timeline_event_ctn')) {
...
$output .= "\n".'<!-- Timeline Event --><div class="timeline_event_ctn '.$type.' clearfix" id="tle_'.$id.'"><div class="v_line"></div>'.do_shortcode($content).'</div>
...
if (!function_exists('timeline_event')) {
...
$img = wpb_getImageBySize( array( 'attach_id' => $img_id, 'thumb_size' => '250'  ) );	
		...
		$output = '<div class="timeline_event '.$animation.'"><div class="event_img">'.$img['thumbnail'].'</div><div class="event_triangle"></div><div class="event_info"><h2>'.$title.'</h2><p>'.do_shortcode($content).'</p></div></div>';
...
if (!function_exists('timeline_date')) {
