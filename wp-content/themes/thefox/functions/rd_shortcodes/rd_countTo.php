...
$output .='<style type="text/css" >#rand_'.$c_id.':hover {background:'.$h_bg.'; box-shadow:0 0 0 15px rgba('.$bs[0].','.$bs[1].','.$bs[2].',0.7); border-color:'.$h_bg.';}#rand_'.$c_id.':hover .count_number{color:'.$h_number_color.' !important; }#rand_'.$c_id.':hover .count_title{color:'.$h_heading_color.' !important; }</style>';
	
}else{


$output .='<style type="text/css" >#rand_'.$c_id.':hover {background:'.$h_bg.'; box-shadow:0 0 0 15px '.$h_bg.'; border-color:'.$h_bg.';}#rand_'.$c_id.':hover .count_number{color:'.$h_number_color.' !important; }#rand_'.$c_id.':hover .count_title{color:'.$h_heading_color.' !important; }</style>';		
...
if($type == '3'){
	...
	$output .='<style type="text/css" >#rand_'.$c_id.' .count_icon_circle{background:'.$i_color.';}#rand_'.$c_id.' .count_number:after{background:'.$i_color.';}</style>';	
...
$output .= '<style type="text/css" >#rand_'.$c_id.' { background:'.$bbg_color.'; border: 1px solid '.$bb_color.'; box-shadow: 0 0px 0px rgba(255, 255, 255, 0), 0 7px 0 -1px '.$bbg_color.', 0 0px 0px 0px '.$bbg_color.',0 0px 0px '.$bbg_color.', 0 7px 0 0px '.$bb_color.';}</style>';
...
if($type == '8'){
	...
	$output .='<style type="text/css" >#rand_'.$c_id.' .count_icon_circle{color:'.$i_color.'; background:'.$i_bg_color.';}#rand_'.$c_id.' .count_icon_circle:before{ background:'.$i_bg_color.';}#rand_'.$c_id.' .count_number:after{background:'.$line_color.';}</style>';	
...
if($type == '10'){
	...
	if($icon !== ''){
		...
		$output .= $ret;
		$output .= '<style type="text/css" >#rand_'.$c_id.' .count_stroke_circle{border:1px solid '.$number_color.'; color:'.$number_color.';}#rand_'.$c_id.':hover .count_stroke_circle{border:1px solid '.$i_color.'; }#rand_'.$c_id.':hover .count_stroke_circle i{color:#fff;}</style>';
...
if($type == '11'){
	...
	if($icon !== ''){
		...
		$output .= $ret;
		$output .= '<style type="text/css" >#rand_'.$c_id.' .count_gradient_circle .count_gradient_bg,#rand_'.$c_id.' .count_gradient_circle i{ background: '.$i_bg_color.'; background: -moz-linear-gradient(-45deg,  '.$i_bg_color.' 0%, '.$i_bg_alt_color.' 100%); background: -webkit-gradient(linear, left top, right bottom, color-stop(0%,'.$i_bg_color.'), color-stop(100%,'.$i_bg_alt_color.')); background: -webkit-linear-gradient(-45deg,  '.$i_bg_color.' 0%,'.$i_bg_alt_color.' 100%); background: -o-linear-gradient(-45deg,  '.$i_bg_color.' 0%,'.$i_bg_alt_color.' 100%); background: -ms-linear-gradient(-45deg,  '.$i_bg_color.' 0%,'.$i_bg_alt_color.' 100%); background: linear-gradient(135deg,  '.$i_bg_color.' 0%,'.$i_bg_alt_color.' 100%); filter: progid:DXImageTransform.Microsoft.gradient( startColorstr="'.$i_bg_color.'", endColorstr="'.$i_bg_alt_color.'",GradientType=1 );}#rand_'.$c_id.' .count_gradient_circle i{border-color:'.$i_color.'; color:'.$i_color.';}#rand_'.$c_id.' .count_number:after{background:'.$heading_color.';}</style>';
...
if($type == '13'){
	...
	if($icon !== ''){
		...
		$output .= $ret;
		$output .= '<style type="text/css" >#rand_'.$c_id.' .count_l_big_circle{color:'.$i_color.'; background:'.$i_bg_color.';}#rand_'.$c_id.':hover .count_l_big_circle{color:'.$i_h_color.'; background:'.$i_bg_h_color.';}</style>';
		$output .= '<div class="count_l_big_circle"><i class="'.$icon.'"></i></div>';
		
	}
		
	$output .='<div class="rd_count_to count_style_13"><div class="count_number" data-from="0" data-to="'.$number.'" data-speed="'.$speed.'" data-refresh-interval="25" data-decimals="'.$decimals.'" style="color:'.$number_color.'">0</div><span class="count_title" style="color:'.$heading_color.'">'.$content.'</span></div></div>';
