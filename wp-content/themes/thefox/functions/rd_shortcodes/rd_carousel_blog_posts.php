...
if (!function_exists('carousel_posts_sc')) {
...
echo '<style type="text/css" >.rand'.$blog_rand_class.'_post .widget_post_title a,.rand'.$blog_rand_class.'_post .rp_date a,.rand'.$blog_rand_class.'_post .rp_date { color:#fff;}.rand'.$blog_rand_class.'_post .rp_date a,.rand'.$blog_rand_class.'_post .rp_date {padding-bottom:0; margin-bottom:0;}.rand'.$blog_rand_class.'_post .blog_box_content {position:absolute; bottom: 20px ;min-width: 100%; text-align: center;}.cbp_type01 .recent_port_ctn:hover .blog_box_content {bottom:85px;}.recent_port_ctn:hover .blog_img_link,.recent_port_ctn:hover .blog_post_link{bottom:35px}</style>';	
}
...
echo '<style type="text/css" >.rd_'.$rp_rs.' .post-title h2 a{color:'.$h_color.';}.rd_'.$rp_rs.' .post-info,.rd_'.$rp_rs.' .post-info a,.rd_'.$rp_rs.' .entry{color:'.$t_color.';}.rd_'.$rp_rs.' .post-title h2 a:hover{color:'.$hover_color.';}.rd_'.$rp_rs.' .post-info a:hover,.rd_'.$rp_rs.' .entry a:hover{color:'.$hover_color.';}</style>';	
}

if ($type == 'cbp_type05' || $type == 'cbp_type06'  || $type == 'cbp_type08'  ){
echo '<style type="text/css" >.rand'.$blog_rand_class.'_post .blog_box_content{background:'.$bg_color.'}</style>';	
}
	...
	//setup up Carousel
		...
		$(".rd_'.$rp_rs.'.rp_sc ul").carouFredSel({
					responsive: true,
					width: "100%",
						height: "variable",';
					if($scroll !== 'yes' ){ 
