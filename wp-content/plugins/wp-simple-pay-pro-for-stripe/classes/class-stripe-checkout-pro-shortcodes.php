...
if ( ! class_exists( 'Stripe_Checkout_Shortcodes' ) ) {
	...
	class Stripe_Checkout_Shortcodes {
	   ...
	   function stripe_shortcode( $attr, $content = null ) {
		   ...
		   // If we are here and a charge just happened let's exit early
		   ...
		   $sc_script_options['script']['key'] = apply_filters( 'simpay_publishable_key', $sc_script_options['script']['key'], $filter_mode );
		   ...
		   $script_vars[ self::$sc_id ] = array(
				    ...
				    'testMode'            => ( ! empty( $sc_script_options['script']['test_mode'] ) && ! ( $sc_script_options['script']['test_mode'] === 'false' ) ? $sc_script_options['script']['test_mode'] : -1 ),
			   		'invalid_html_string' => __( 'This button has been disabled because the form is not well-formed HTML. Please check your shortcode source code to make sure nothing is conflicting.', 'stripe' ),
		   ...
		   if ( $shipping == 'true' ) {
			   ...
			   $html .= '<input type="hidden" name="sc-shipping-postal-code" class="sc-shipping-postal-code" value="" />';
			   $html .= '<input type="hidden" name="sc-shipping-state" class="sc-shipping-state" value="" />';
			   $html .= '<input type="hidden" name="sc-shipping-line1" class="sc-shipping-line1" value="" />';
