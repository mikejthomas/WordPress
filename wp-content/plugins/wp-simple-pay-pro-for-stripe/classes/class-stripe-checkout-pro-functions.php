	...
	class Stripe_Checkout_Pro_Functions extends Stripe_Checkout_Functions {
		...
		private function __construct() {
			...
			if ( isset( $_POST['stripeToken'] ) && isset( $_POST['wp-simple-pay'] ) ) {
				self::$token = true;
				add_action( 'wp', array( $this, 'charge_card' ), 0 );
			}
		...
		public static function charge_card() {
			...
			if ( self::$token && wp_verify_nonce( $_POST['wp-simple-pay-pro-nonce'], 'charge_card' ) ) {
				...
				// Retrieve shipping data from POST.
				// Don't proceed if no shipping name since it's validated first.
				$shipping_name = ( isset( $_POST['sc-shipping-name'] ) ? $_POST['sc-shipping-name'] : '' );
				$shipping      = array();

				// Save shipping data to array format a Stripe charge is expecting.
				if ( ! empty( $shipping_name ) ) {
					$shipping = array(
						'name'    => $shipping_name,
						'address' => array(
							'country'     => ( isset( $_POST['sc-shipping-country'] ) ? $_POST['sc-shipping-country'] : '' ),
							'postal_code' => ( isset( $_POST['sc-shipping-postal-code'] ) ? $_POST['sc-shipping-postal-code'] : '' ),
							'state'       => ( isset( $_POST['sc-shipping-state'] ) ? $_POST['sc-shipping-state'] : '' ),
							'line1'       => ( isset( $_POST['sc-shipping-line1'] ) ? $_POST['sc-shipping-line1'] : '' ),
							'city'        => ( isset( $_POST['sc-shipping-city'] ) ? $_POST['sc-shipping-city'] : '' ),
						),
					);
				}
				...
				if ( has_action( 'sc_do_charge' ) ) {
					...
					try {
						...
						// Add shipping data to charge args if exists.
						if ( ! empty( $shipping_name ) ) {
							$charge_args['shipping'] = $shipping;
						}
